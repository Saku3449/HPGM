...
def visualize(boxes_coord, boxes_type, save_pic_path):
    ...
    vutils.save_bbox(background, boxes_collection,
                     save_pic_path, normalize=True, draw_line=True)
...
def draw_point(points_hulls, colors, save_img_path, center=False):
    ...
    for k in range(len(points_hulls)):
        ...
        for i in range(len(point_hulls)):
            image = cv.circle(image, (int(point_hulls[i][0]), int(
                point_hulls[i][1])), 2, colors[k], -1)
    if center:
        point = get_center_point(point_hulls)
        image = cv.circle(
            image, (int(point[0]), int(point[1])), 2, (0, 0, 255))
        ...
        for i in range(point_hull.shape[0]):
            ...
            if point_hull[i][0] == point_hull[compare][0]:
                ...
                if last_type == "x_same":
                    ...
                    point_hull = np.insert(point_hull, compare, [
                                           [point_hull[compare][0], point_hull[i][1]]], 0)
                    break
                elif last_type == "y_same":
                    point_hull = np.insert(point_hull, compare, [
                                           [point_hull[i][0], point_hull[compare][1]]], 0)
                    break
        if i + 1 != point_hull.shape[0]:
            last_type = "decide which is the same"
    return point_hull
...
def get_line_intersection(line1, line2, cross_point):
    ...
    # a X (a1 - b1) same sign with a X b
    if s_numer != 0 and (s_numer < 0) == de_norm_positive:
        return 0
...
def check_intersection_polygon(intersection_point, contour_coord, direction, num=1):
    ...
    [x, y, w, h] = cv.boundingRect(
        np.array(intersection_point).astype(np.float32))
...
def get_intersection_polygon(bounding_box_point, contour_coord, direction, num=1):
    ...
    """
    line = [[bounding_box_point[1], bounding_box_point[2]],
            [bounding_box_point[2], bounding_box_point[3]]]
    ...
    if len(intersection_box_point) > 3:
        intersection_box_point, delete_point = check_intersection_polygon(
            intersection_box_point, contour_coord, direction, num)
...
def clock_wise(point_hull):
    ...
    point_hull = sorted(
        point_hull, key=functools.cmp_to_key(clockwise_sort.compare))
...
def get_room_contour(hull_conversion, boxes_coord, pair_id=1):
    ...
    contour_point_hull, contour_bounding_box = hull_conversion.get_contour_layout(
        boxes_coord, first=True, contour=True)
    ...
    for i in range(room_num):
        ...
        while len(point_coords) < 20:
            point_coords = point_split_scale(contour_coord, bounding_box)
        random_room_contour, rest_contour, pair_point = hull_conversion.random_room_generate(
            point_coords, contour_coord, seed=pair_id*4+i)
        pair_points.append(pair_point)
        # random the room point hull
        room_point_hull, room_bounding_box = hull_conversion.get_contour_layout(
            random_room_contour, first=False, contour=False)
        room_coords.append(room_point_hull)
        room_bounding_boxes.append(room_bounding_box)
        contour_coord, _ = hull_conversion.get_contour_layout(
            rest_contour, first=False, contour=True)
...
class ConversionLayout(object):
    ...
    def random_room_generate(self, contour_coord, random_point):
        ...
        bounding_box_point = self.build_bounding_box_point(
            random_point, direction)
        ...
        if intersection_box_point is None or len(intersection_box_point) == 1:
            ...
            return None
        first_contour_random_point = np.asarray(
            intersection_box_point).astype(np.float32)
        ...
        for i in range(len(contour_coord)):
            ...
            if cv.pointPolygonTest(bounding_box_point, (contour_coord[i][0], contour_coord[i][1]), False) == +1:
                ...
                first_contour_random_point = np.vstack(
                    (first_contour_random_point, contour_coord[i]))
        ...
        if len(first_contour_random_point) < 3:
            ...
            return None
        first_contour_random_point = check_contour_hull(
            first_contour_random_point)
        # /***** new generate second contour layout by shapely *****/
        from shapely.ops import split
        from shapely.geometry import Polygon, LineString
        intersection_box_point = self.delete_redundancy_element(
            intersection_box_point)
        intersection_box_point = np.round(intersection_box_point)
        # if len(intersection_box_point) != 3:
        #     print("error happen! please check the intersection box point")
        line = LineString([intersection_box_point[0],
                          intersection_box_point[-1], intersection_box_point[1]])
        ...
        if len(contour_collection) > 1:
            second_contour_random_point = np.array(
                contour_collection[0].exterior.coords).astype(np.int32)
            first_contour_random_point = np.array(
                contour_collection[1].exterior.coords).astype(np.int32)
        else:
            # print("error happen Please check!")
            return None
            # assert False
            second_contour_random_point = second_contour_random_point.astype(
                np.int32)
            first_contour_random_point = first_contour_random_point.astype(
                np.int32)
        ...
        random_room_contour = self.delete_redundancy_element(
            random_room_contour)
    ...
    def get_contour_layout(boxes_coord, first=True, contour=False):
        ...
        if first:
            point_set = np.zeros(
                (boxes_coord.shape[0] * 2, 2), dtype=np.float32)
            ...
            bounding_box = np.array(
                [bounding_box[0], bounding_box[1], bounding_box[2], bounding_box[3]], dtype=np.int32)
            if contour:
    ...
    def tran_layout_hull(self, layout_init_contour, layout):
        ...
        def judge_area(room_num, room_hull, init_contour):
            ...
            def calculate_surface_area(polygon):
                ...
                im = np.zeros((256, 256))
                polygon_mask = cv.fillPoly(
                    im, [np.array(polygon, dtype=np.int32)], 255)
                area = np.sum(np.greater(polygon_mask, 0))
                return area
        ...
        for i in range(room_num):
            ...
            random_point = [int(layout[i][0] * 255),
                            int(layout[i][1] * 255), int(layout[i][2] * 4)]
            ...
            # random the room point hull
            room_point_hull, room_bounding_box = self.get_contour_layout(
                random_room_contour, first=False, contour=False)
            room_coords.append(room_point_hull)
            # make the rest_contour be in clockwise
            contour_coord, _ = self.get_contour_layout(
                rest_contour, first=False, contour=True)
        room_coords.append(contour_coord)
        for i in range(len(room_coords)):
            k = judge_area(len(room_coords),
                           room_coords[i], layout_init_contour*255)
            if k == False:
                return -1
        return room_coords
...
if __name__ == '__main__':
    ...
    for i in range(1, 401):  # 1, 401
        data_boxes_path = "./dataset/data/first_layer_data/layout{}_boxes_coord.txt".format(
            i)
        boxes_coord = np.round(np.loadtxt(data_boxes_path), decimals=4)
        layer_pair_points = []
        layer_pair_hulls = []
        for n in range(pair_nums):
            point_3d_hulls, point_bounding_boxes, pair_points = get_room_contour(
                hull_conversion, boxes_coord, n)
        ...
        save_train_data_path = "./multiLayer_data/pair_train_data/layers_pair_points_{}.npy".format(
            i)
        save_hull_data_path = "./multiLayer_data/pair_hull_data/layers_hull_points_{}.npy".format(
            i)
