...
def generate_triangle(triangle_index, point_hull, shift_index=0):
    ...
    if point_hull is None:
        ...
        for i in range(triangle.shape[0]):
            triangle[i] = [triangle_index[0]+shift_index,
                           triangle_index[i+1]+shift_index, triangle_index[i+2]+shift_index]
    else:
        triangle = np.zeros((len(triangle_index) - 2, 3))
        for i in range(triangle.shape[0]):
...
def generate_line_set(point_hull, bottom_height, height, max_size):
    ...
    vertical_link_1, vertical_link_2 = np.zeros(
        (point_num, 3)), np.zeros((point_num, 3))
    # downside_link = np.zeros((point_num, 3))
    upside_link_1, upside_link_2 = np.zeros(
        (point_num, 3)), np.zeros((point_num, 3))
    upside_back_link_1 = np.zeros((point_num, 3))
    contour_3d_hull_1, contour_3d_hull_2 = np.zeros(
        (point_num * 3, 3)), np.zeros((point_num * 3, 3))
    for i in range(point_num):
        point = point_hull[i]
        contour_3d_hull_1[3 * i] = [point[0] / max_size,
                                    point[1] / max_size, bottom_height/max_size]
        contour_3d_hull_1[3 * i + 1] = [point[0] / max_size,
                                        point[1] / max_size, height / max_size]
        contour_3d_hull_1[3 * i + 2] = [point[0] / max_size +
                                        0.002, point[1] / max_size+0.002, height / max_size]
        vertical_link_1[i] = [3*i, 3*i+1, 3*i+2]
        upside_link_1[i] = [3*i+1, 3*i+2, (3*i+4) % (point_num*3)]
        upside_back_link_1[i] = [
            (3*i+4) % (point_num*3), (3*i+5) % (point_num*3), 3*i+2]
    for i in range(point_num):
        point = point_hull[i]
        contour_3d_hull_2[3 * i] = [point[0] / max_size,
                                    point[1] / max_size, bottom_height/max_size]
        contour_3d_hull_2[3 * i + 1] = [point[0] / max_size+0.002,
                                        point[1] / max_size+0.002, bottom_height / max_size]
        contour_3d_hull_2[3 * i + 2] = [point[0] / max_size,
                                        point[1] / max_size, height / max_size]
    ...
    lines_link_2 = np.vstack(
        (vertical_link_2+contour_3d_hull_1.shape[0], upside_link_2+contour_3d_hull_2.shape[0]))
...
def generate_3d_contour(point_hull, height, bottom_height, shift_triangle_index, max_size, surface_type):
    ...
    for i in range(point_num):
        point = point_hull[i]
        contour_3d_hull[2 * i] = [point[0] / max_size,
                                  point[1] / max_size, bottom_height/max_size]
        contour_3d_hull[2 * i + 1] = [point[0] / max_size,
                                      point[1] / max_size, height / max_size]
    # generate every surface triangle
    triangle_index = [i for i in range(contour_3d_hull.shape[0])]
    surface_index = [triangle_index[i:i + 4]
                     for i in range(0, len(triangle_index), 2)]
    ...
    if surface_type == "downside":
        contour_3d_triangle = generate_triangle(
            downside_index, point_hull, shift_index=shift_triangle_index)
    elif surface_type == "upside":
        contour_3d_triangle = generate_triangle(
            upside_index, point_hull, shift_index=shift_triangle_index)
    else:
        downside_3d_triangle = generate_triangle(
            downside_index, point_hull, shift_index=shift_triangle_index)
        upside_3d_triangle = generate_triangle(
            upside_index, point_hull, shift_index=shift_triangle_index)
        contour_3d_triangle = np.vstack(
            (downside_3d_triangle, upside_3d_triangle))
    # get the surface_triangle
    for i in range(len(surface_index)):
        surface_triangle = generate_triangle(
            surface_index[i], None, shift_index=shift_triangle_index)
        contour_3d_triangle = np.vstack(
            (contour_3d_triangle, surface_triangle))
...
def generate_wall(point_hull, shift_triangle_index, bottom_height, height, contour_point_hull, wall_thickness=3, max_size=256):
    ...
    for i in range(point_num):
        ...
        if point[1] == adjacent_point[1]:
            wall_point_hull[i][2] = [adjacent_point[0],
                                     adjacent_point[1] - wall_thickness]
            wall_point_hull[i][3] = [point[0], point[1] - wall_thickness]
        elif point[0] == adjacent_point[0]:
            wall_point_hull[i][2] = [adjacent_point[0] -
                                     wall_thickness, adjacent_point[1]]
    ...
    for i in range(len(wall_point_hull)):
        wall_3d_hull, wall_3d_triangle = generate_3d_contour(
            wall_point_hull[i], height, bottom_height, shift_triangle_index, max_size, "upside")
        if i == 0:
            wall_3d_hulls = wall_3d_hull
            wall_3d_triangles = wall_3d_triangle
        else:
            wall_3d_hulls = np.vstack((wall_3d_hulls, wall_3d_hull))
            wall_3d_triangles = np.vstack(
                (wall_3d_triangles, wall_3d_triangle))
...
def render_3d_contour(init_hull, first_hull, Next_hull, save_mesh_path):
    ...
    Color = np.array([[84, 139, 84], [0, 100, 0], [0, 0, 128], [85, 26, 139], [255, 0, 255],
                      [165, 42, 42], [139, 134, 130], [
                          205, 198, 115], [139, 58, 58],
    ...
    for l in range(layout_layer):
        ...
        for h in range(len(point_3d_hulls[l])):
            Bottom_3d_hulls, Bottom_3d_triangle = generate_3d_contour(
                point_3d_hulls[0][h],  bottom_height+1, bottom_height, 0, max_size, "Both")
            Bottom_layout.vertices = open3d.utility.Vector3dVector(
                Bottom_3d_hulls)
            Bottom_layout.triangles = open3d.utility.Vector3iVector(
                Bottom_3d_triangle)
        ...
        for n in range(len(point_3d_hulls[l])):
            ...
            wall_hulls, wall_triangles = generate_wall(
                point_hull, 0, bottom_height, height, contour_point_hull, wall_thickness=wall_thick)
    ...
    layout_hulls, layout_triangles = generate_line_set(
        contour_point_hull, 0, height, max_size)
...
if __name__ == "__main__":
    def load_data_path():
        ...
        num = 48
        init_hull_path = os.path.join(
            abosulte_path, "{}/layout_init_hulls_{}.npy".format(data_dir, num))
        # /***for cover experiment******/
        first_hull_path = os.path.join(
            abosulte_path, "{}/layout_gt_hulls_{}.npy".format(data_dir, num))
        ...
        w_all_hull_path = os.path.join(
            abosulte_path, "{}/layout_score_hulls_{}.npy".format(data_dir, num))
    ...
    for l in range(layout_layer):
        ...
        if l == 1:
            for h in range(len(point_3d_hulls[l+1])):
                Bottom_3d_hulls, Bottom_3d_triangle = generate_3d_contour(
                    point_3d_hulls[l+1][h],  bottom_height+1, bottom_height, 0, max_size, "Both")
                Bottom_layout.vertices = open3d.utility.Vector3dVector(
                    Bottom_3d_hulls)
                Bottom_layout.triangles = open3d.utility.Vector3iVector(
                    Bottom_3d_triangle)
        ...
        for h in range(len(point_3d_hulls[l])):
            Bottom_3d_hulls, Bottom_3d_triangle = generate_3d_contour(
                point_3d_hulls[l][h],  bottom_height+1, bottom_height, 0, max_size, "Both")
            Bottom_layout.vertices = open3d.utility.Vector3dVector(
                Bottom_3d_hulls)
            Bottom_layout.triangles = open3d.utility.Vector3iVector(
                Bottom_3d_triangle)
        ...
        lines_vertices, lines_link = generate_line_set(
            contour_point_hull, bottom_height, height)
    ...
    open3d.visualization.draw_geometries(
        [layouts]+lines, mesh_show_back_face=True)
