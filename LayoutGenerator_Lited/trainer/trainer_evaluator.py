...
def visualization(point_hulls, pred_score, save_pic_path, contour_type=None, data_type="numpy", draw_text=False):
    ...
    def visualize(boxes_collection):
        ...
        im = vutils.save_bbox(background, boxes_collection,
                              save_pic_path, normalize=True, draw_line=True, save=False)
        return im
    images = None
    hull_collection = []
    ...
    for i in range(len(point_hulls)):
        ...
        for n in range(len(point_hulls[i])):
            if n <= len(contour_type)-1:
                ...
                type = 10.0
            hull_coord, hull_type = trans_hull_boxes_coord(
                point_hulls[i][n], type, data_type)
            for k in range(len(hull_coord)):
                hull_coords.append(hull_coord[k])
                hull_types.append(hull_type[k])
        # for visualization
        boxes_type = [torch.tensor(hull_types[t])
                      for t in range(len(hull_types))]
...
class LayoutTrainer(object):
    ...
    def define_models(self):
        ...
        evaluator = LayoutEvaluator(
            room_dim, room_hiddern_dim, score_hiddern_dim, bidirectional=False)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            train_label = torch.FloatTensor([[-1.0]])
            ...
            for step, data in enumerate(self.dataloader_train, 0):
                ...
                layout1 = torch.cat(
                    (layout1_init_contour, layout1_room), dim=1).transpose(0, 1)
                layout2 = torch.cat(
                    (layout2_init_contour, layout2_room), dim=1).transpose(0, 1)
                pred_score1 = layout_evaluator(layout1)
                pred_score2 = layout_evaluator(layout2)
                indicator_values = indicator_values[0].reshape(
                    [len(indicator_values[0]), 1]).float()
                scale = indicator_values.shape[0]
                pred = torch.sum((pred_score1 - pred_score2)
                                 * indicator_values) / scale
                if step == 299:
                    self.logger.info("In epoch {}, pred_score1: {}, pred_score2: {}".format(
                        epoch, pred_score1[0], pred_score2[0]))
            ...
            with torch.no_grad():
                ...
                test_label = torch.FloatTensor([[-1.0]])
                test_label = torch.FloatTensor([[-1.0]])
                ...
                for step, data in enumerate(self.dataloader_test, 0):
                    ...
                    test_layout1_init_contour, test_layout2_init_contour = test_init_contour[
                        0], test_init_contour[0]
                    test_layout1 = torch.cat(
                        (test_layout1_init_contour, test_layout1_room), dim=1).transpose(0, 1)
                    test_layout2 = torch.cat(
                        (test_layout2_init_contour, test_layout2_room), dim=1).transpose(0, 1)
                    test_pred_score1 = layout_evaluator.forward(test_layout1)
                    test_pred_score2 = layout_evaluator.forward(test_layout2)
                    test_indicator_values = test_indicator_values[0].reshape(
                        [len(test_indicator_values[0]), 1]).float()
                    scale = test_indicator_values.shape[0]
                    test_pred = torch.sum(
                        (test_pred_score1 - test_pred_score2) * test_indicator_values) / scale
    ...
    def evaluate(self):
        ...
        room_classes_size_range = [6, 4, 8, 7, 2,
                                   5, 3, 1, 0]  # according to the class
        ...
        with open(save_score_path, "w") as score_file:
            for step, data in enumerate(self.dataloader_test, 0):
                ...
                for i in range(save_num):
                    eval_layout1_hull, eval_layout2_hull = eval_pair_hulls[
                        0][i][0], eval_pair_hulls[0][i][1]
                    eval_layout1_hull, new_contour_type = room_size_range_hull(
                        eval_layout1_hull, contour_types[0], room_classes_size_range)
                    eval_layout2_hull, new_contour_type = room_size_range_hull(
                        eval_layout2_hull, contour_types[0], room_classes_size_range)
                    pred_score = [eval_score1[i][0], eval_score2[i][0]]
                    save_path = os.path.join(
                        self.eval_dir, "layout_score_{}_{}.png".format(num, i))
                    visualization([eval_layout1_hull, eval_layout2_hull], pred_score,
                                  save_path, new_contour_type, "none", draw_text=True)
